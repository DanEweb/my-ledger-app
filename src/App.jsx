...
import { ChevronsLeft, ChevronsRight, PlusCircle, CreditCard, Home, ShoppingCart, Utensils, Zap, Repeat, Target, Settings, LayoutDashboard, Wallet, FileOutput, Landmark, Trophy, Gift, Sparkles, User, Tag, TrendingUp, TrendingDown, CircleDollarSign, Sun, Moon, Search, X, Calendar, MoreHorizontal } from 'lucide-react';
...
const initialAppData = {
  admin: { 
    ...
    assets: [
      { id: 'asset-1', name: '주거래 은행', balance: 50000000, type: 'bank'},
      { id: 'asset-debt-1', name: '학자금 대출', balance: -15000000, type: 'liability'}
    ], 
    ...
    userCoupons: [],
    rewardSettings: { 'chal-1': '아이스크림 교환권' }
  },
  invitedUser1: { 
...
const Dashboard = ({ userData, t, onAddTransaction, setActiveView }) => {
    const { assets = [], transactions = [], recurringTransactions = [], budgets = [], goals = [] } = userData;
    const netAssets = useMemo(() => assets.reduce((sum, asset) => sum + asset.balance, 0), [assets]);
    const totalLiabilities = useMemo(() => assets.filter(a => a.balance < 0).reduce((sum, asset) => sum + asset.balance, 0), [assets]);
    return (
        <div className="space-y-6">
            <header className="flex justify-between items-center">
                <div>
                 ...
                 <button onClick={onAddTransaction} className="flex items-center bg-teal-500 text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-teal-600">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm"> <h3 className="font-semibold text-slate-800 dark:text-slate-200">{t.netAssets}</h3> <p className="text-3xl font-bold text-blue-500 mt-2">{formatCurrency(netAssets)}</p> </div>
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm"> <h3 className="font-semibold text-slate-800 dark:text-slate-200">{t.totalLiabilities}</h3> <p className="text-3xl font-bold text-red-500 mt-2">{formatCurrency(totalLiabilities)}</p> </div>
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm sm:col-span-2"> <h3 className="font-semibold text-slate-800 dark:text-slate-200 mb-4">{t.budgets}</h3> <div className="space-y-4"> {(budgets.length > 0) ? budgets.slice(0, 2).map(b => { const percentage = b.budget > 0 ? Math.round((b.spent / b.budget) * 100) : 0; return (<div key={b.id}> <div className="flex justify-between text-sm mb-1"> <span className="font-medium text-slate-600 dark:text-slate-300">{b.category}</span> <span className="text-slate-500">{formatCurrency(b.spent)} / {formatCurrency(b.budget)}</span> </div> <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2"><div className="bg-teal-500 h-2 rounded-full" style={{ width: `${percentage}%` }}></div></div> </div>); }) : <p className="text-sm text-slate-400">설정된 예산이 없습니다.</p>} </div> </div>
                </div>
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm flex flex-col"> <div className="flex justify-between items-center mb-4"> <h3 className="font-semibold text-slate-800 dark:text-slate-200">{t.recentHistory}</h3></div> <div className="space-y-3 flex-grow"> {(transactions.length > 0) ? [...transactions].sort((a,b) => new Date(b.date) - new Date(a.date)).slice(0, 5).map(tx => ( <div key={tx.id} className="flex items-center justify-between text-sm"> <div className="flex items-center gap-3"> <div className={`p-2 rounded-full ${CATEGORY_COLORS[tx.category] || 'bg-gray-100'}`}>{getIcon(tx.category)}</div> <div> <p className="font-medium text-slate-700 dark:text-slate-300">{tx.description}</p> <p className="text-xs text-slate-400">{tx.date}</p> </div> </div> <p className={`font-semibold ${tx.type === 'income' ? 'text-green-500' : 'text-red-500'}`}>{tx.type === 'income' ? '+' : '-'}{formatCurrency(tx.amount)}</p> </div> )) : <p className="text-sm text-slate-400 text-center pt-8">내역이 없습니다.</p>} </div> <button onClick={() => setActiveView('history')} className="mt-4 text-sm w-full text-center py-2 rounded-lg bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 font-semibold text-slate-600 dark:text-slate-300">{t.showMore}</button> </div>
            </div>
        </div>
    );
};
const Assets = ({ assets, t, onAddAsset }) => ( <div className="space-y-6"> <header className="flex justify-between items-center"> <div><h2 className="text-3xl font-bold text-slate-900 dark:text-slate-100">{t.assetsTitle}</h2><p className="text-slate-600 dark:text-slate-400 mt-1">{t.assetsDesc}</p></div> <button onClick={onAddAsset} className="flex items-center bg-teal-500 text-white font-semibold py-2 px-4 rounded-lg shadow-sm hover:bg-teal-600"><PlusCircle className="h-5 w-5 mr-2" />{t.addAsset}</button> </header> <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{(assets || []).map(asset => (<div key={asset.id} className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm"><div className="flex items-center gap-3"><div className={`p-3 rounded-full ${asset.balance < 0 ? 'bg-red-100 dark:bg-red-900/50' : 'bg-blue-100 dark:bg-blue-900/50'}`}>{getAssetIcon(asset.type)}</div><h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200">{asset.name}</h3></div><p className={`text-3xl font-bold mt-4 ${asset.balance < 0 ? 'text-red-500' : 'text-blue-500'}`}>{formatCurrency(asset.balance)}</p></div>))}</div> </div> );
            ...
            <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-2xl shadow-sm">
                ...
                <div className="space-y-2">
                    {filteredTransactions.map(tx => (
                         <div key={tx.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50"><div className="flex items-center gap-3"><div className={`p-2 rounded-full ${CATEGORY_COLORS[tx.category] || 'bg-gray-100'}`}>{getIcon(tx.category)}</div><div><p className="font-semibold text-slate-700 dark:text-slate-300">{tx.description}</p><p className="text-sm text-slate-400">{tx.date}</p></div></div><p className={`font-semibold ${tx.type === 'income' ? 'text-green-500' : 'text-red-500'}`}>{tx.type === 'income' ? '+' : '-'}{formatCurrency(tx.amount)}</p></div>
                    ))}
                </div>
            </div>
        </div>
...
const AddGoalModal = ({ isOpen, onClose, onSubmit, t }) => { const handleSubmit = (e) => { e.preventDefault(); const formData = new FormData(e.target); const data = { name: formData.get('name'), target: parseFloat(formData.get('target')), saved: 0, id: `goal-mock-${Date.now()}` }; onSubmit(data); }; return ( <Modal isOpen={isOpen} onClose={onClose} title={t.newGoal}> <form onSubmit={handleSubmit} className="space-y-4"> <div><label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">{t.goalName}</label><input type="text" name="name" placeholder="예: 유럽 여행 가기" required className="w-full p-2 rounded-md bg-slate-100 dark:bg-slate-700 border-transparent focus:ring-2 focus:ring-teal-500" /></div> <div><label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">{t.targetAmount}</label><input type="number" name="target" placeholder="5000000" step="1" required className="w-full p-2 rounded-md bg-slate-100 dark:bg-slate-700 border-transparent focus:ring-2 focus:ring-teal-500" /></div> <div className="flex justify-end gap-3 pt-4"><button type="button" onClick={onClose} className="px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-200 bg-slate-200 dark:bg-slate-600 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-500">{t.cancel}</button><button type="submit" className="px-4 py-2 text-sm font-semibold text-white bg-teal-500 rounded-lg hover:bg-teal-600 shadow-sm">{t.add}</button></div> </form> </Modal> ); };
